...
# Get indices for training and testing ranges
train_indices = []
for year in range(2000, 2005):
    train_indices.extend(range(year_trace[year][0], year_trace[year][1]))

for year in range(2011, 2023):
    train_indices.extend(range(year_trace[year][0], year_trace[year][1]))

#train_indices.append(year_trace[2022][1])
test_indices = []
for year in range(2005, 2010):
    test_indices.extend(range(year_trace[year][0], year_trace[year][1]))

#test_indices.append(year_trace[2015][1])
#test_indices.append(year_trace[2015][1])
...
x_train = total_images[train_indices[:-1]]
y_train = total_images[train_indices[1:]]

x_test = total_images[test_indices[:-1]]
y_test = total_images[test_indices[1:]]

print(f"Training data from {2000} to {2010} and {2017} to {2022}:")
print("x_train shape:", x_train.shape)
print("y_train shape:", y_train.shape)

print(f"Testing data from {2011} to {2015}:")
...
for epoch in range(epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), 'best_vae_model_2005_2010_early.pth')
...
path_2016_22.load_state_dict(torch.load('best_vae_model_2005_2010_early.pth', map_location=torch.device('cpu')))
